...
app.get(
  '/command/:command',
  (req, res) => {
    res.sendFile(path.join(__dirname, "views", "command.html"), );
  });
...
app.get("/api/bot-info", async (req, res) => {
  try {
    ...
    const botActivities = client.presence.activities[0]['state'];
...
app.post("/api/update-bot", async (req, res) => {
  const { newBotName, newBotActivities } = req.body;
  ...
  if(newBotActivities) {
    try {
      ...
      data['activities'] = {
        value: newBotActivities
      };
      const Stringdata = JSON.stringify(data)
      fs.writeFileSync('./config.json', Stringdata);
      client.user.setPresence({
        activities: [{
          type: ActivityType.Custom,
          name: "custom",
          state: newBotActivities,
        }]
      });
    } catch (error) {
      console.error("Error updating bot activities name:", error);
      return res.json({ success: false, message: "Failed to update bot activities" });
    }
  }
...
app.post("/update-config", (req, res) => {
  ...
  const newConfig = {
    ...
    activities: {
      ...
      ...(req.body.activities && { value: req.body.activities }),
    }
  };
